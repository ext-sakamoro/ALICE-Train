...
use crate::qwen35::{DeltaNetLayerWeights, FullAttnLayerWeights, Qwen35Config, Qwen35LayerWeights, LayerType};
use std::fs;
...
/// DeltaNet レイヤーを FP32 バイナリから読み込み。
...
/// Full Attention レイヤーを FP32 バイナリから読み込み。
...
pub fn build_cache(
    ...
    for i in 0..config.num_hidden_layers {
        ...
        if (i + 1) % 8 == 0 || i == config.num_hidden_layers - 1 {
            eprintln!("    FP32 キャッシュ: {}/{} 層変換完了", i + 1, config.num_hidden_layers);
        }
    }
...
pub fn load_layer_from_cache(
    ...
    match lt {
        LayerType::LinearAttention => {
            Ok(Qwen35LayerWeights::DeltaNet(load_deltanet_layer(&path, config)?))
        }
        LayerType::FullAttention => {
            Ok(Qwen35LayerWeights::FullAttention(load_fullattn_layer(&path, config)?))
        }
    }
}
...
mod tests {
    ...
    fn test_cache_roundtrip_deltanet() {
        let config = Qwen35Config {
            ...
            linear_key_head_dim: 4, linear_num_key_heads: 2,
            linear_value_head_dim: 4, linear_num_value_heads: 2,
            linear_conv_kernel_dim: 2, full_attention_interval: 4,
