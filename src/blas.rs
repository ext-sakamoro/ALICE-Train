//! BLAS matmul — CUDA cuBLAS / macOS Accelerate / フォールバック タイル matmul。
//!
//! L9: projection matmul を 50-100x 高速化。
//!
//! - CUDA (feature `cuda` + `init_cuda_blas()` 呼出後): cuBLAS TF32 Tensor Cores
...
#[cfg(feature = "cuda")]
pub static CUDA_MATMUL: OnceLock<Mutex<crate::cuda_matmul::CudaMatmul>> = OnceLock::new();

/// CUDA cuBLAS を初期化し、以降の `blas_matmul_bt` / `blas_matmul_nn` で GPU を使用する。
#[cfg(feature = "cuda")]
pub fn init_cuda_blas() {
    CUDA_MATMUL.get_or_init(|| {
        let cuda = crate::cuda_matmul::CudaMatmul::new();
        Mutex::new(cuda)
    });
}
...
/// CUDA が初期化済みかどうか。
#[cfg(feature = "cuda")]
pub fn cuda_blas_available() -> bool {
    CUDA_MATMUL.get().is_some()
}
...
pub fn blas_matmul_bt(a: &[f32], b: &[f32], c: &mut [f32], m: usize, n: usize, k: usize) {
    ...
    if let Some(cuda_mtx) = CUDA_MATMUL.get() {
        ...
        cuda.matmul_bt_inplace(a, b, c, m, n, k);
        return;
    }
...
pub fn blas_matmul_nn(a: &[f32], b: &[f32], c: &mut [f32], m: usize, n: usize, k: usize) {
    ...
    if let Some(cuda_mtx) = CUDA_MATMUL.get() {
        let cuda = cuda_mtx.lock().expect("CUDA mutex poisoned");
        cuda.matmul_nn_inplace(a, b, c, m, n, k);
        return;
    }
