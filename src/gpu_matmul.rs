...
impl GpuMatmul {
    ...
    pub fn new(ctx: &GpuContext) -> Self {
        ...
        let (matmul_pipeline, matmul_layout) = Self::build_pipeline(device, MATMUL_BT_SHADER, "matmul_bt", &[
        ...
        let (add_pipeline, add_layout) = Self::build_pipeline(device, ADD_SHADER, "add", elem_entries);
        let (mul_pipeline, mul_layout) = Self::build_pipeline(device, MUL_SHADER, "mul", elem_entries);
        ...
        let (silu_pipeline, silu_layout) = Self::build_pipeline(device, SILU_SHADER, "silu", silu_entries);

        Self { matmul_pipeline, matmul_layout, add_pipeline, add_layout, silu_pipeline, silu_layout, mul_pipeline, mul_layout }
    }
    ...
    pub fn swiglu_ffn(
        ...
        let silu_params = ElemParams { len: total as u32, _pad: [0; 3] };
        ...
        let silu_bg = ctx.device().create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: buf_gate.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: buf_silu_params.as_entire_binding() },
            ],
        });

        // Mul: gate *= up
        let mul_params = ElemParams { len: total as u32, _pad: [0; 3] };
        ...
        let mul_bg = ctx.device().create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: buf_gate.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: buf_up.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 2, resource: buf_mul_params.as_entire_binding() },
            ],
        });
        ...
        self.matmul_bt(ctx, &intermediate, down_proj, seq_len, hidden_dim, intermediate_dim)
    }
}
...
pub fn gpu_layer_forward(
    ...
    let q = gpu.matmul_bt(ctx, &normed, &weights.q_proj, seq_len, num_heads * head_dim, hidden_dim);
    let k = gpu.matmul_bt(ctx, &normed, &weights.k_proj, seq_len, num_kv_heads * head_dim, hidden_dim);
    let v = gpu.matmul_bt(ctx, &normed, &weights.v_proj, seq_len, num_kv_heads * head_dim, hidden_dim);
    ...
    let attn_out = gpu.matmul_bt(ctx, &attn_out_raw, &weights.o_proj, seq_len, hidden_dim, num_heads * head_dim);
    ...
    let gate = gpu.matmul_bt(ctx, &normed_ffn_buf, &weights.gate_proj, seq_len, intermediate_dim, hidden_dim);
    let up = gpu.matmul_bt(ctx, &normed_ffn_buf, &weights.up_proj, seq_len, intermediate_dim, hidden_dim);
