...
#[cfg(feature = "cuda")]
use crate::deltanet::deltanet_recurrence_forward_eval_cuda;
use crate::deltanet::{
...
use crate::qwen35::{DeltaNetLayerWeights, FullAttnLayerWeights, Qwen35Config, Qwen35LayerWeights};
...
/// 1レイヤーの eval forward (cache なし, in-place)。gradient checkpointing 用。
pub fn qwen35_layer_forward_eval_inplace(
    input: &mut Vec<f32>,
    weights: &Qwen35LayerWeights,
    config: &Qwen35Config,
    seq_len: usize,
    ...
    match weights {
        Qwen35LayerWeights::DeltaNet(w) => {
            deltanet_layer_forward_eval(input, w, config, seq_len);
        }
        Qwen35LayerWeights::FullAttention(w) => {
            full_attn_layer_forward_eval(input, w, config, seq_len);
        }
    }
}
...
fn deltanet_layer_forward_eval(
    ...
    #[cfg(feature = "cuda")]
    {
        if crate::blas::cuda_blas_available() {
            deltanet_recurrence_forward_eval_cuda(
                &q_expanded,
                &k_expanded,
                &v_all,
                &beta,
...
pub fn qwen35_model_forward_eval_streaming(
    ...
    // L12: FP32 キャッシュ有無で分岐
    let use_cache = cache_dir.is_some_and(|d| crate::fp32_cache::cache_exists(d, config));

    for i in 0..config.num_hidden_layers {
        let layer = if use_cache {
            // FP32 キャッシュから読み込み (BF16デコード不要)
            crate::fp32_cache::load_layer_from_cache(cache_dir.unwrap(), i, config).unwrap_or_else(
                |e| {
