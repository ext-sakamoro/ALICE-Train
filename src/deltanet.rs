        ...
        .for_each(|(t, row_out)| {
            for c in 0..channels {
                ...
                for k in 0..kernel_size {
                    let src_t = t as isize - (kernel_size as isize - 1) + k as isize;
                    if src_t >= 0 {
                        sum = x[src_t as usize * channels + c].mul_add(w[k], sum);
                    }
                }
                row_out[c] = silu(sum);
            }
    ...
    for c in 0..channels {
        ...
        for t in 0..seq_len {
            ...
            for k in 0..kernel_size {
                let src_t = t as isize - (kernel_size as isize - 1) + k as isize;
                if src_t >= 0 {
                    conv_out = x[src_t as usize * channels + c].mul_add(w[k], conv_out);
                }
            }
            ...
            for k in 0..kernel_size {
                ...
                if src_t >= 0 {
                    ...
                    d_x[src * channels + c] += d_conv * w[k];
                    d_weight[c * kernel_size + k] += d_conv * x[src * channels + c];
                }
            }
        }
    }
...
pub fn head_recurrence_forward_eval(
    ...
    for t in 0..seq_len {
        ...
        // retrieve
        for j in 0..dv { r_buf[j] = 0.0; }
        for di in 0..dk {
            let k_di = k[di];
            let row = &state[di * dv..(di + 1) * dv];
            for dj in 0..dv { r_buf[dj] = row[dj].mul_add(k_di, r_buf[dj]); }
        }
        ...
        for dj in 0..dv { o[dj] = 0.0; }
        for di in 0..dk {
            let q_di = q[di];
            let row = &state[di * dv..(di + 1) * dv];
            for dj in 0..dv { o[dj] = row[dj].mul_add(q_di, o[dj]); }
        }
    }
}
...
fn head_recurrence_forward_chunked(
    ...
    // チャンク境界の状態のみ保存
    let mut chunk_boundary_states: Vec<Vec<f32>> = Vec::with_capacity(num_chunks + 1);
    chunk_boundary_states.push(state.to_vec()); // 初期状態
    ...
    for chunk_idx in 0..num_chunks {
        ...
        for t in chunk_start..chunk_end {
            ...
            // error
            for j in 0..dv { e_buf[j] = v[j] - r_buf[j]; }
        ...
        // チャンク境界の状態を保存
        chunk_boundary_states.push(state.to_vec());
    }
...
pub fn deltanet_recurrence_forward(
    ...
    for t in 0..seq_len {
        for h in 0..num_heads {
            per_head_q[h][t * dk..(t + 1) * dk]
                .copy_from_slice(&q_all[t * num_heads * dk + h * dk..t * num_heads * dk + (h + 1) * dk]);
            per_head_k[h][t * dk..(t + 1) * dk]
                .copy_from_slice(&k_all[t * num_heads * dk + h * dk..t * num_heads * dk + (h + 1) * dk]);
            per_head_v[h][t * dv..(t + 1) * dv]
                .copy_from_slice(&v_all[t * num_heads * dv + h * dv..t * num_heads * dv + (h + 1) * dv]);
...
fn head_recurrence_backward(
    ...
    for t in (0..seq_len).rev() {
        ...
        for di in 0..dk {
            ...
            for dj in 0..dv {
                s_t[di * dv + dj] = cache.exp_g.mul_add(
                    cache.s_prev[di * dv + dj],
                    bk * cache.e[dj],
                );
            }
        }
        ...
        for i in 0..dk * dv {
            frobenius = ds[i].mul_add(cache.s_prev[i], frobenius);
        }
...
pub fn deltanet_recurrence_backward(
    ...
    for t in 0..seq_len {
        for h in 0..num_heads {
            per_head_do[h][t * dv..(t + 1) * dv]
                .copy_from_slice(&d_output[t * num_heads * dv + h * dv..t * num_heads * dv + (h + 1) * dv]);
        }
    }
...
mod tests {
    ...
    fn test_deltanet_single_step_parallel() {
        ...
        let (caches, _) = deltanet_recurrence_forward(
            &q, &k, &v, &beta, &g, &b_logits, &a_logits, &mut output, 1, 2, 2, 1,
        );
    ...
    fn test_deltanet_backward_gradient_check() {
        ...
        let (caches, _) = deltanet_recurrence_forward(
            &q, &k, &v, &beta, &g, &b_logits, &a_logits, &mut output, num_heads, dk, dv, seq_len,
        );

        let d_output = vec![1.0f32; seq_len * num_heads * dv];
        let grads = deltanet_recurrence_backward(
            &d_output, &caches, &a_log_val, &dt_bias_val, num_heads, dk, dv, seq_len,
        );
