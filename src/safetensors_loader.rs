...
impl ShardedModel {
    ...
    pub fn open<P: AsRef<Path>>(model_dir: P) -> io::Result<Self> {
        ...
        let (weight_map, shard_files) = if index_path.exists() {
            ...
            let shard_files: Vec<String> = index.weight_map.values().cloned().collect::<std::collections::HashSet<_>>().into_iter().collect();
            ...
            if !single_path.exists() {
                return Err(io::Error::new(
                    ...
                    format!("safetensors ファイルが見つかりません: {}", model_dir.display()),
                ));
            }
        ...
        let weight_map = if weight_map.is_empty() {
            ...
            let st = SafeTensors::deserialize(mmap)
                .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, format!("safetensors パースエラー: {e}")))?;
            st.names().into_iter().map(|name| (name.to_string(), single_name.clone())).collect()
        } else {
            weight_map
        };

        eprintln!("  ShardedModel: {} テンソル, {} シャード", weight_map.len(), shards.len());
