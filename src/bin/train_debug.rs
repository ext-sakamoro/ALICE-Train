...
use alice_train::{DataLoader, DataLoaderConfig, LrScheduler, MmapDataset, WarmupCosineScheduler};
...
fn main() {
    ...
    logln("╚══════════════════════════════════════════════════════════╝");
    logln(&format!(
        "seq_len={}, total_steps={}, layers={}",
        config.seq_len, config.total_steps, config.model.num_layers
    ));
    ...
    for step in 0..config.total_steps {
        ...
        log("  [proj load] ");
        let t0 = Instant::now();
        let output_proj = get_tensor("lm_head.weight")
            .unwrap_or_else(|| get_tensor("model.embed_tokens.weight").unwrap());
        logln(&format!(
            "{:.1}ms ({:.0} MB)",
            t0.elapsed().as_secs_f64() * 1000.0,
            output_proj.len() as f64 * 4.0 / 1e6
        ));
        ...
        logln(&format!(
            "{:.1}ms ({} tokens)",
            logits_time.as_secs_f64() * 1000.0,
            token_count
        ));
        ...
        for h in 0..hidden_dim {
            output_norm[h] -=
                lr * (d_output_norm_w[h] * inv_tokens + config.weight_decay * output_norm[h]);
        }
        logln(&format!("{:.1}ms", t0.elapsed().as_secs_f64() * 1000.0));
        ...
        for i in (0..num_layers).rev() {
            ...
            let load_ms = lt.elapsed().as_millis();
            let bt = Instant::now();
            let (d_in, _weight_grads) =
                layer_backward(&d_layer_input, &caches[i], &lw, &config.model, seq_len);
        ...
        logln(&format!(
            "\n  ═══ step {step} | loss={avg_loss:.4} | lr={lr:.2e} | total={:.1}s ═══",
            step_total.as_secs_f64()
        ));
        logln(&format!(
            "    fwd layers: {:.1}s | bwd layers: {:.1}s",
            fwd_total.as_secs_f64(),
            bwd_layers_time.as_secs_f64()
        ));
        logln("");
    }
