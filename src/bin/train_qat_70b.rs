...
use alice_train::{
    ...
    LogEntry, LossScaler, LrScheduler, MixedPrecisionConfig, MmapDataset, OffloadConfig, QatConfig,
    TrainLog, WarmupCosineScheduler,
};
...
fn main() {
    ...
    let dataset = MmapDataset::open(train_data_path).unwrap_or_else(|e| {
        eprintln!("[ALICE-Train] データ読み込み失敗: {train_data_path}: {e}");
        eprintln!();
        eprintln!("データを準備してください:");
        eprintln!("  cargo run --release --features qat-cli --bin prepare-data -- --all");
        std::process::exit(1);
    });
    ...
    if phase1_mode {
        ...
        use alice_train::cuda_matmul::{
            cuda_layer_backward_ws_vram, cuda_layer_forward_eval_ws,
            cuda_layer_forward_eval_ws_vram, cuda_layer_forward_ws_vram, CudaLayerWorkspace,
            VramLayerWeights,
        };
        ...
        let base_weights: Vec<LlamaLayerWeights> = if config.preload_all_layers {
            ...
            let weights: Vec<LlamaLayerWeights> = (0..num_layers)
                .map(|i| {
                    LlamaLayerWeights::from_tensors(i, &get_tensor, &config.model).unwrap_or_else(
                        || {
        ...
        fn get_base_layer(
            ...
            if !base_weights.is_empty() {
                ...
                LlamaLayerWeights::from_tensors(layer_idx, get_tensor, config).unwrap_or_else(
                    || {
        ...
        if config.bf16_delta {
            ...
            for i in 0..num_layers {
                let f32_cache =
                    LayerDeltaCache::load_from_disk(i, &config.model, &config.checkpoint_dir);
                bf16_delta_store.push(Bf16DeltaStore::from_f32(&f32_cache));
                // f32_cache は即 drop — 1層分のFP32しかピークに載らない
            }
            delta_cache = Vec::new();
            println!(
                "    delta キャッシュ初期化完了 (BF16 圧縮: {:.1} GB)",
        ...
        println!(
            "    CudaLayerWorkspace: {:.1} MB (forward+backward共通バッファ)",
            ws_mb
        );

        // メモリ見積もり
        let embed_mb = (embedding_table.len() + output_proj.len() + output_norm.len()) as f64 * 4.0
            / 1024.0
            / 1024.0;
        let layers_mb = (num_layers * layer_bytes) as f64 / 1024.0 / 1024.0;
        let delta_mb = if config.bf16_delta {
            layers_mb / 2.0
        } else {
            layers_mb
        };
        ...
        let mut working_delta = if config.bf16_delta {
            Some(LayerDeltaCache::load_from_disk(
                0,
                &config.model,
                &config.checkpoint_dir,
            ))
        ...
        println!(
            "  AdamW 状態: {:.1} MB/layer × {} layers = {:.1} MB",
        ...
        while global_step < config.total_steps {
            ...
            for _mb in 0..accum_steps {
                ...
                for b in 0..batch.actual_batch_size {
                    let token_ids: Vec<u32> =
                        batch.input_ids[b * seq_len..(b + 1) * seq_len].to_vec();
                    let targets: Vec<u32> =
                        batch.target_ids[b * seq_len..(b + 1) * seq_len].to_vec();
                    let mut hidden = vec![0.0f32; seq_len * hidden_dim];
                    for (t, &tok) in token_ids.iter().enumerate() {
                        let tok = tok as usize;
                        if tok < vocab_size {
                            hidden[t * hidden_dim..(t + 1) * hidden_dim].copy_from_slice(
            ...
            for i in 0..num_layers {
                ...
                if use_bf16_delta {
                    ...
                    let t1 = Instant::now();
                    wd.fused_merge_and_quantize_alpha(
                        &base_lw,
                        &mut workspace_lw,
                        &mut fq,
                        &mut quantize_buf,
                    ...
                    fwd_expand_ms += t0.elapsed().as_millis();
                    let t1 = Instant::now();
                    delta_cache[i].fused_merge_and_quantize_alpha(
                        &base_lw,
                        &mut workspace_lw,
                        &mut fq,
                        &mut quantize_buf,
                ...
                if global_step == 0 {
                    ...
                    if any_nan || any_inf {
                        ...
                        // 注目レイヤーは正常でも出力
                        let w_min = workspace_lw
                            .q_proj
                            .iter()
                            .cloned()
                            .fold(f32::INFINITY, f32::min);
                        let w_max = workspace_lw
                            .q_proj
                            .iter()
                            .cloned()
                    ...
                    if i == 0 {
                        ...
                        let h_max = all_hiddens[0]
                            .iter()
                            .cloned()
                            .fold(f32::NEG_INFINITY, f32::max);
                        let h_nan = all_hiddens[0].iter().any(|x| x.is_nan());
                        println!(
                            "    [DEBUG] hidden pre-L0: min={h_min:.6} max={h_max:.6} nan={h_nan}"
                        );
                    }
                }
            ...
            for b in 0..total_samples {
                ...
                if b == 0 && global_step == 0 {
                    ...
                    if target0 < vocab_size {
                        let max_l = l_max;
                        let sum_exp: f64 =
                            logits_t0.iter().map(|&x| ((x - max_l) as f64).exp()).sum();
                ...
                if b == 0 && global_step == 0 {
                    let d_logits_norm: f64 = d_logits_flat
                        .iter()
                        .map(|&x| (x as f64) * (x as f64))
                        .sum::<f64>()
                        .sqrt();
                    let d_logits_max = d_logits_flat
                        .iter()
                        .cloned()
                        .fold(0.0f32, |a, b| a.max(b.abs()));
                    println!(
                        "    [DEBUG] d_logits: L2={d_logits_norm:.6e} max_abs={d_logits_max:.6e}"
                    );
                }
            ...
            // d_logits で既に Mean 化済みなので inv_tokens 不要
            {
                ...
                for h in 0..hidden_dim {
                    ...
                    output_norm[h] -= effective_lr
                        * (m_hat / (v_hat.sqrt() + eps) + config.weight_decay * output_norm[h]);
                }
            }
            drop(all_hiddens);
            drop(all_token_ids);
            drop(all_targets);
            println!(
                "    [TIMING] Phase C Loss+OutBwd: {}ms",
                phase_c_start.elapsed().as_millis()
            );
            ...
            for i in (0..num_layers).rev() {
                ...
                if use_bf16_delta {
                    ...
                    let t1 = Instant::now();
                    let (mae, cos, cnt) = wd.fused_merge_and_quantize_alpha(
                        &base_lw,
                        &mut workspace_lw,
                        &mut fq,
                        &mut quantize_buf,
                    ...
                    bwd_expand_ms += t0.elapsed().as_millis();
                    let t1 = Instant::now();
                    let (mae, cos, cnt) = delta_cache[i].fused_merge_and_quantize_alpha(
                        &base_lw,
                        &mut workspace_lw,
                        &mut fq,
                        &mut quantize_buf,
                ...
                for b in 0..total_samples {
                    ...
                    let cache = cuda_layer_forward_ws_vram(
                        &cuda,
            ...
            for i in 0..num_layers {
                ...
                if let Some(ref mut b) = g.q_bias {
                    layer_nan_count += sanitize(b);
                }
                if let Some(ref mut b) = g.k_bias {
                    layer_nan_count += sanitize(b);
                }
                if let Some(ref mut b) = g.v_bias {
                    layer_nan_count += sanitize(b);
                }
            ...
            for i in 0..num_layers {
                ...
                for field in [
                    ...
                    global_grad_sq_sum +=
                        field.iter().map(|&x| (x as f64) * (x as f64)).sum::<f64>();
                }
            }
            ...
            if clip_scale < 1.0 {
                ...
                for i in 0..num_layers {
                    let g = &mut accumulated_grads[i];
                    for v in g.attn_norm.iter_mut() {
                        *v *= s;
                    }
                    for v in g.q_proj.iter_mut() {
                        *v *= s;
                    }
                    for v in g.k_proj.iter_mut() {
                        *v *= s;
                    }
                    for v in g.v_proj.iter_mut() {
                        *v *= s;
                    }
                    for v in g.o_proj.iter_mut() {
                        *v *= s;
                    }
                    for v in g.ffn_norm.iter_mut() {
                        *v *= s;
                    }
                    for v in g.gate_proj.iter_mut() {
                        *v *= s;
                    }
                    for v in g.up_proj.iter_mut() {
                        *v *= s;
                    }
                    for v in g.down_proj.iter_mut() {
                        *v *= s;
                    }
                }
            }
                ...
                && eval_dataset.is_some()
            {
                ...
                for eb in 0..eval_batches {
                    ...
                    for (t, &tok) in token_ids.iter().enumerate() {
                        let tok = tok as usize;
                        if tok < vocab_size {
                            hidden_b[t * hidden_dim..(t + 1) * hidden_dim].copy_from_slice(
                                &embedding_table[tok * hidden_dim..(tok + 1) * hidden_dim],
                            );
                        }
                    }
                ...
                for i in 0..num_layers {
                    ...
                    if use_bf16_delta {
                        ...
                        delta_cache[i].fused_merge_and_quantize(
                            &base_lw,
                            &mut workspace_lw,
                            &mut fq,
                            &mut quantize_buf,
                    ...
                    for eb in 0..eval_batches {
                        cuda_layer_forward_eval_ws_vram(
                            &cuda,
                            &mut all_hiddens[eb],
                            &workspace_lw,
                            &vram_w,
            ...
            if global_step > 0 && global_step % config.checkpoint_interval == 0 {
                ...
                println!(
                    "  チェックポイント保存: {ckpt_path} (delta+adam+resume {:.1}s)",
                    ckpt_start.elapsed().as_secs_f32()
                );
            }
        ...
        let resume_json =
            format!("{{\"step\":{global_step},\"loss\":{final_loss:.6},\"lr\":{final_lr:.2e}}}",);
        let resume_path = format!("{}/resume_state.json", config.checkpoint_dir);
        let _ = std::fs::write(&resume_path, &resume_json);
...
impl LayerAdamState {
    ...
    fn zeros(config: &LlamaConfig) -> Self {
        ...
        Self {
            attn_norm_m: vec![0.0; hidden],
            attn_norm_v: vec![0.0; hidden],
            q_proj_m: vec![0.0; q_dim * hidden],
            q_proj_v: vec![0.0; q_dim * hidden],
            k_proj_m: vec![0.0; kv_dim * hidden],
            k_proj_v: vec![0.0; kv_dim * hidden],
            v_proj_m: vec![0.0; kv_dim * hidden],
            v_proj_v: vec![0.0; kv_dim * hidden],
            o_proj_m: vec![0.0; hidden * q_dim],
            o_proj_v: vec![0.0; hidden * q_dim],
            ffn_norm_m: vec![0.0; hidden],
            ffn_norm_v: vec![0.0; hidden],
            gate_proj_m: vec![0.0; inter * hidden],
            gate_proj_v: vec![0.0; inter * hidden],
            up_proj_m: vec![0.0; inter * hidden],
            up_proj_v: vec![0.0; inter * hidden],
            down_proj_m: vec![0.0; hidden * inter],
            down_proj_v: vec![0.0; hidden * inter],
            q_bias_m,
            q_bias_v,
            k_bias_m,
            k_bias_v,
    ...
    fn save_to_disk(&self, layer_idx: usize, checkpoint_dir: &str) {
        ...
        if let Some(ref d) = self.q_bias_m {
            sv("adam_q_bias_m", d);
        }
        if let Some(ref d) = self.q_bias_v {
            sv("adam_q_bias_v", d);
        }
        if let Some(ref d) = self.k_bias_m {
            sv("adam_k_bias_m", d);
        }
        if let Some(ref d) = self.k_bias_v {
            sv("adam_k_bias_v", d);
        }
        if let Some(ref d) = self.v_bias_m {
            sv("adam_v_bias_m", d);
        }
        if let Some(ref d) = self.v_bias_v {
            sv("adam_v_bias_v", d);
        }
    }
    ...
    fn load_from_disk(layer_idx: usize, config: &LlamaConfig, checkpoint_dir: &str) -> Self {
        ...
        {
            (
                Some(ld("adam_q_bias_m", q_dim)),
                Some(ld("adam_q_bias_v", q_dim)),
                Some(ld("adam_k_bias_m", kv_dim)),
                Some(ld("adam_k_bias_v", kv_dim)),
                Some(ld("adam_v_bias_m", kv_dim)),
                Some(ld("adam_v_bias_v", kv_dim)),
            )
        } else {
            (None, None, None, None, None, None)
        };
    ...
    fn memory_bytes(&self) -> usize {
        ...
        if let Some(ref v) = self.q_bias_m {
            total += count(v) * 2;
        }
        if let Some(ref v) = self.k_bias_m {
            total += count(v) * 2;
        }
        if let Some(ref v) = self.v_bias_m {
            total += count(v) * 2;
        }
        total
    }
...
impl LayerDeltaCache {
    ...
    fn fused_merge_and_quantize_alpha(
        ...
        let pairs: Vec<(&[f32], &[f32], &mut [f32])> = vec![
            ...
            (
                &base.gate_proj,
                &self.gate_proj,
                workspace.gate_proj.as_mut_slice(),
            ),
            (
                &base.up_proj,
                &self.up_proj,
                workspace.up_proj.as_mut_slice(),
            ),
            (
                &base.down_proj,
                &self.down_proj,
                workspace.down_proj.as_mut_slice(),
            ),
        ];
        ...
        for (b, d, w) in pairs {
            ...
            let (sum_err, dot_wq, dot_ww, dot_qq) = w
                .par_iter_mut()
                .map(|wv_mut| {
        ...
        workspace
            .attn_norm
            .par_iter_mut()
            .zip(base.attn_norm.par_iter().zip(self.attn_norm.par_iter()))
            .for_each(|(out, (&b, &d))| *out = b + d);
        workspace
            .ffn_norm
            .par_iter_mut()
            .zip(base.ffn_norm.par_iter().zip(self.ffn_norm.par_iter()))
            .for_each(|(out, (&b, &d))| *out = b + d);

        // Bias
        if let (Some(ref mut wb), Some(ref bb), Some(ref db)) =
            (&mut workspace.q_bias, &base.q_bias, &self.q_bias)
        {
            wb.iter_mut()
                .zip(bb.iter().zip(db.iter()))
                .for_each(|(out, (&b, &d))| *out = b + d);
        }
        if let (Some(ref mut wb), Some(ref bb), Some(ref db)) =
            (&mut workspace.k_bias, &base.k_bias, &self.k_bias)
        {
            wb.iter_mut()
                .zip(bb.iter().zip(db.iter()))
                .for_each(|(out, (&b, &d))| *out = b + d);
        ...
        if let (Some(ref mut wb), Some(ref bb), Some(ref db)) =
            (&mut workspace.v_bias, &base.v_bias, &self.v_bias)
        {
            wb.iter_mut()
                .zip(bb.iter().zip(db.iter()))
                .for_each(|(out, (&b, &d))| *out = b + d);
    ...
    fn apply_adam(
        ...
        let adamw_update =
            |delta: &mut [f32], grad: &[f32], m: &mut [f32], v: &mut [f32], wd: f32| {
        ...
        rayon::scope(|s| {
            ...
            s.spawn(|_| {
                adamw_update(
                    &mut self.gate_proj,
                    &grads.gate_proj,
                    &mut adam.gate_proj_m,
                    &mut adam.gate_proj_v,
                    wd,
                )
            });
            s.spawn(|_| {
            ...
            s.spawn(|_| {
                adamw_update(
                    &mut self.down_proj,
                    &grads.down_proj,
                    &mut adam.down_proj_m,
                    &mut adam.down_proj_v,
                    wd,
                )
            });
        });
        ...
        adamw_update(
            &mut self.attn_norm,
            &grads.attn_norm,
            &mut adam.attn_norm_m,
            &mut adam.attn_norm_v,
            weight_decay,
        );
        adamw_update(
            &mut self.ffn_norm,
    ...
    fn apply_adam_gpu(
        ...
        grads: &LayerWeightGrads,
        adam: &mut LayerAdamState,
        cuda: &alice_train::cuda_matmul::CudaMatmul,
        lr: f32,
        weight_decay: f32,
        step: usize,
    ) {
    ...
    fn save_to_disk(&self, layer_idx: usize, checkpoint_dir: &str) {
        ...
        if let Some(ref d) = self.q_bias {
            sv("q_bias", d);
        }
        if let Some(ref d) = self.k_bias {
            sv("k_bias", d);
        }
        if let Some(ref d) = self.v_bias {
            sv("v_bias", d);
        }
    }
}
...
fn load_layer_delta(
    ...
    let path = format!(
        "{}/delta_layer{layer_idx}_{weight_name}.bin",
        checkpoint_dir
    );
